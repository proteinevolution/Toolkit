<template>
    <div>
        <b-row class="mb-2">
            <b-col cols="12" sm="5" md="3" lg="4" xl="3">
                <label class="d-flex align-items-center">
                    <span v-text="$t('jobs.results.hitlist.table.perPage.show')"></span>
                    <b-form-select v-model="perPage" :options="perPageOptions" class="mx-2" />
                    <span v-text="$t('jobs.results.hitlist.table.perPage.entries')"></span>
                </label>
            </b-col>
            <b-col cols="12" sm="7" md="6" offset-md="3" offset-lg="2" xl="4" offset-xl="5">
        ...
        <div v-show="totalRows > perPage" class="pagination-container">
            <span
                v-if="totalNoFilter === totalRows"
                v-text="$t('jobs.results.hitlist.table.paginationInfo', { start, end, total: totalRows })"></span>
            <span
                v-else
                v-text="
                    $t('jobs.results.hitlist.table.paginationInfoFiltered', { start, end, totalRows, totalNoFilter })
                "></span>
            <b-pagination
                v-model="currentPage"
    ...
    methods: {
        hitsProvider(ctx: any) {
            ...
            return resultsService
                .fetchHits(this.job.jobID, start, end, ctx.filter, ctx.sortBy, ctx.sortDesc)
