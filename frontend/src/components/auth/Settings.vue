<template>
    <b-form @submit.prevent="changePassword">
        <b-form-group :label="$t('auth.changePassword')">
            <b-form-input v-model="oldPassword" type="password" :placeholder="$t('auth.oldPassword')" />
        </b-form-group>
        ...
        <b-btn :disabled="!valid" type="submit" v-text="$t('auth.changePassword')" />
    </b-form>
</template>
...
export default defineComponent({
    ...
    computed: {
        ...
        valid(): boolean {
            return this.oldPassword !== '' && this.newPasswordValid && this.confirmPasswordValid;
        },
    },
    methods: {
        validateNewPassword() {
