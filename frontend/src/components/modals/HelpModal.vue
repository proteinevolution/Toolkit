<template>
    <BaseModal id="helpModal" :title="tool ? tool.longname : ''">
        <b-tabs v-if="toolName && languageLoaded">
            <b-tab class="helpTab" title="Overview" v-html="$t(`toolHelpModals.${toolName}.overview`)" />
            <b-tab class="helpTab" title="Input & Parameters">
                <Accordion :items="accordionItems" />
            </b-tab>
            <b-tab class="helpTab" title="References">
                <div v-html="$t(`citation`)"></div>
                <br />
                <div v-html="$t(`toolHelpModals.${toolName}.references`)"></div>
            </b-tab>
            <b-tab
                v-if="tool && tool.version"
                class="helpTab"
                title="Version"
...
export default defineComponent({
    ...
    watch: {
        toolName(name) {
            ...
            loadExtraTranslations(`modals/toolHelp/${name}`).finally(() => {
