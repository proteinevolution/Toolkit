...
final case class HTTPRequest(req: RequestHeader) {
  ...
  def isSynchronousHttp: Boolean =
    !isXhr && !isSocket

  def isSafe: Boolean =
    req.method == "GET"

  def isRedirectable: Boolean =
  ...
  def isTrident: Boolean = uaContains("Trident/")
  def isChrome: Boolean  = uaContains("Chrome/")
  def isSafari: Boolean  = uaContains("Safari/") && !isChrome
