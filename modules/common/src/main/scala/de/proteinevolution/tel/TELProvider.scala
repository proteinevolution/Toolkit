...
import de.proteinevolution.tel.param.{
  GenerativeParamFileParser,
  ParamCollector
}
...
class WrapperPathProvider @Inject()(configuration: Configuration)
    extends TELProvider {
  ...
  override def get(): String = {
    ...
    configuration.get[Option[String]]("tel.wrapper").getOrElse {
      ...
      logger.warn(
        s"Key 'tel.wrapper' was not found in configuration. Fall back to '$fallBackFile'"
      )
      fallBackFile
    }
  }
...
class RunscriptPathProvider @Inject()(configuration: Configuration)
    extends TELProvider {
  ...
  override def get(): String = {
    ...
    configuration.get[Option[String]]("tel.runscripts").getOrElse {
      ...
      logger.warn(
        s"Key 'tel.runscripts' was not found in configuration. Fall back to '$fallBackFile'"
      )
      fallBackFile
    }
  }
...
class ParamCollectorProvider @Inject()(
  ...
  override def get(): ParamCollector = {
    ...
    lazy val paramFilePath =
      configuration.get[Option[String]]("tel.params").getOrElse {

        val fallBackFile = "tel/paramspec/PARAMS"
        logger.warn(
          s"Key 'tel.params' was not found in configuration. Fall back to '$fallBackFile'"
        )
        fallBackFile
      }
