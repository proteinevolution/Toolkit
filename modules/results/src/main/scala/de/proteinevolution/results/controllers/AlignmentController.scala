...
class AlignmentController @Inject()(
  ...
  def getAln(jobID: String): Action[AnyContent] = Action.async {
    implicit request =>
  ...
  def loadHits(jobID: String): Action[AnyContent] = Action.async {
    implicit request =>
      val json       = request.body.asJson.get
      ...
      resultFiles.getResults(jobID).map {
        ...
        case Some(jsValue) =>
          ...
          if (end > result.alignment.length || start > result.alignment.length) {
            ...
            val hits = result.alignment
              .slice(start, end)
              .map(views.html.alignment.alignmentrow(_))
            Ok(hits.mkString)
          }
      }
  }

  def loadHitsClustal(jobID: String): Action[AnyContent] = Action.async {
    implicit request =>
      val json       = request.body.asJson.get
      val resultName = (json \ "resultName").as[String]
      ...
      resultFiles.getResults(jobID).map {
        ...
        case Some(jsValue) =>
          val result = aln.parse((jsValue \ resultName).as[JsArray])
          val hits =
            Common.clustal(result, 0, constants.breakAfterClustal, color)
          Ok(hits.mkString)
      }
  }
}
