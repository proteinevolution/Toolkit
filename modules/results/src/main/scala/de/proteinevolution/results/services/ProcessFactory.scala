...
private[results] object ProcessFactory {
  ...
  def apply(
    ...
    val retrieveFullSeqHHblits =
      (basePath + "/retrieveFullSeqHHblits.sh").toFile // why so little abstractions ???
    ...
    val (script, params) = (toolName, mode.toString) match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          "no valid parameters for processing a forwarding job"
        )
    }
