...
@Singleton
class BackendDao @Inject()(private val reactiveMongoApi: ReactiveMongoApi)(
  ...
  def updateStats(
    ...
    statisticsCol.flatMap(
      _.findAndUpdate(
        selector =
          BSONDocument(StatisticsObject.IDDB -> statisticsObject.statisticsID),
  ...
  lazy val loadStatisticsCollection: Future[BSONCollection] = {
    reactiveMongoApi.database.map(
      _.collection[BSONCollection]("loadStatistics")
    )
  }
