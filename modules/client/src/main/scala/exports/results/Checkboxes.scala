...
class Checkboxes(private val outerContainer: JQuery) {
  ...
  val checkedValues: mutable.Map[String, Boolean] =
  ...
  def getChecked: js.Array[Int] =
    checkedValues.filter(_._2).keys.map(_.toInt).toJSArray

  def populateCheckboxes(container: JQuery): Unit = {
    for ((value, checked) <- checkedValues)
      container
        .find(s"input[type=checkbox][name=alignment_elem][value=$value]")
    ...
    container
      ...
      .on(
        ...
        (e: Event) => {
          ...
          val currentState =
            e.currentTarget.asInstanceOf[HTMLInputElement].checked
          checkedValues(currentVal.toString) = currentState // force string value
          // link checkboxes with same value
          container
            .find(
              s"input[type=checkbox][name=alignment_elem][value=$currentVal]"
            )
