...
class AuthController @Inject()(
                  ...
                  if (databaseUser
                    ...
                    userSessions.modifyUserWithCache(selector, modifier).map {
                      case Some(loggedInUser) =>
                        ...
                        wsActorCache.get[List[ActorRef]](
                          unregisteredUser.userID.stringify
                        ) match {
                          case Some(wsActors) =>
                            val actorList: List[ActorRef] =
                              wsActors: List[ActorRef]
                            wsActorCache.set(loggedInUser.userID.stringify,
                                             actorList)
                            actorList.foreach(
                              _ ! ChangeSessionID(loggedInUser.sessionID.get)
                            )
                            wsActorCache
                              .remove(unregisteredUser.userID.stringify)
                          case None =>
                        }
            ...
            signUpFormUser => {
              if (signUpFormUser.accountType < User.NORMALUSERAWAITINGREGISTRATION) {
                ...
                val selector = BSONDocument(
                  "$or" -> List(
                    BSONDocument(
                      User.EMAIL -> signUpFormUser.getUserData.eMail
                    ),
                    BSONDocument(
                      User.NAMELOGIN -> signUpFormUser.getUserData.nameLogin
                    )
                  )
                )
                userDao.findUser(selector).flatMap {
                  ...
                  case None =>
                    ...
                    val newUser = signUpFormUser.copy(
                      ...
                      userToken =
                        Some(
                          UserToken(tokenType = User.REGISTEREDUSER,
                                    eMail =
                                      Some(signUpFormUser.getUserData.eMail))
                        )
                    )
                    userDao.upsertUser(newUser).map {
                      case Some(registeredUser) =>
                        ...
                        registeredUser.userToken match {
                          case Some(token) =>
                            val eMail = NewUserWelcomeMail(registeredUser,
                                                           token.token,
                                                           environment,
                                                           env)
                            eMail.send
            ...
            case Some(user) =>
              user.userData match {
                case Some(_) =>
                  ...
                  // create a modifier document to change the last login date in the Database
                  val bsonCurrentTime =
                    BSONDateTime(ZonedDateTime.now.toInstant.toEpochMilli)
                  ...
                  userSessions.modifyUserWithCache(selector, modifier).map {
                    case Some(registeredUser) =>
                      // All done. User is registered, now send the welcome eMail
                      val eMail =
                        ResetPasswordMail(registeredUser,
                                          token.token,
                                          environment: play.Environment,
                                          env: Env)
                      eMail.send
  ...
  def resetPasswordChange: Action[AnyContent] = Action.async {
    implicit request =>
              ...
              user.userToken match {
                case Some(token) =>
                  if (token.tokenType == 4 && token.userID.isDefined) {
                    ...
                    userSessions
                      .modifyUserWithCache(selector, modifier)
                      .flatMap {
                        case Some(userWithUpdatedAccount) =>
                          userSessions
                            .modifyUserWithCache(
                              BSONDocument(
                                User.IDDB -> userWithUpdatedAccount.userID
                              ),
                              BSONDocument(
                                "$unset" ->
                                BSONDocument(User.USERTOKEN -> "")
  ...
  def passwordChangeSubmit(): Action[AnyContent] = Action.async {
    implicit request =>
      userSessions.getUser.flatMap { user: User =>
        user.userData match {
          case Some(_) =>
              ...
              .fold(
                ...
                // when there are no errors, then insert the user to the collection
                {
                  case Some(newPasswordHash) =>
                    // Generate a new Token to wait for the confirmation eMail
                    val token = UserToken(tokenType = 2,
                    ...
                    userSessions.modifyUserWithCache(selector, modifier).map {
                      case Some(updatedUser) =>
                        ...
                        val eMail =
                          ChangePasswordMail(updatedUser,
                                             token.token,
                                             environment: play.Environment,
                                             env: Env)
                        eMail.send
                ...
                case Some(editedProfileUserData) =>
                  ...
                  val modifier = BSONDocument(
                    ...
                    BSONDocument(User.USERDATA -> editedProfileUserData
                                   .copy(nameLogin = userData.nameLogin),
                                 User.DATELASTLOGIN -> bsonCurrentTime,
                                 User.DATEUPDATED   -> bsonCurrentTime)
                  )

                  if (editedProfileUserData.eMail != user.getUserData.eMail) {
                    val selectorMail = BSONDocument(
                      BSONDocument(User.EMAIL -> editedProfileUserData.eMail)
                    )
