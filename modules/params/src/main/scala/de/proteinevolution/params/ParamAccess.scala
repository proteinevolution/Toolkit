  ...
  def getParam(
    ...
    case "HHPRED_DB_PROTEOMES" =>
      HHpredSelectsParameter(
        "hhsuitedb",
        "Select database (PDB_mmCIF70 for modeling)",
    ...
    case "HHPRED_INCL_EVAL" =>
      select("hhpred_incl_eval", "E-value incl. threshold for MSA generation", default = Some("1e-3"))
    case "MIN_COV" =>
      NumberParameter("min_cov", "Min. coverage of MSA hits (%)", min = Some(0), max = Some(100), default = Some(20))
    ...
    case "MAX_HHBLITS_ITER" =>
      select("max_hhblits_iter", "MSA enrichment iterations using HHblits", default = Some("1"))
    ...
    case "BONUSSCORE" =>
      NumberParameter("bonusscore", "Bonus Score", min = Some(0), max = Some(10), step = Some(0.01), default = Some(0))
    ...
    case "OFFSET" =>
      NumberParameter("offset", "Offset", min = Some(0), max = Some(10), step = Some(0.01), default = Some(0.0))

    // HHrepID
    case "MSA_GEN_MAX_ITER_HHREPID" =>
      select("msa_gen_max_iter_hhrepid", "Maximal no. of MSA generation steps", default = Some("3"))
    ...
    case "SELF_ALN_PVAL_THRESHOLD" =>
      select("self_aln_pval_threshold", "Self-Alignment P-value threshold", default = Some("1e-1"))
