...
final class DatabaseMonitor @Inject()(val reactiveMongoApi: ReactiveMongoApi,
  ...
  private def deleteOldUsers(verbose : Boolean = false): Unit = {
    ...
    val registeredUserDeletionEMailDate     = now.minusMonths(userLoggedInDeletingAfterMonths)
                                                 .plusDays(userLoggedInWarningDaysBeforeDeletion)
    // Date to delete the Registered account at
    val registeredUserDeletionDateForEmail  = now.plusDays(userLoggedInWarningDaysBeforeDeletion)
    ...
    mongoStore.findUsers(
      ...
      mongoStore.removeUsers(BSONDocument(User.IDDB ->BSONDocument("$in" -> userIDs))).foreach{ writeResult =>
        if (verbose)
          Logger.info(s"[User Deletion] Deleting of ${users.length} old users ${if (writeResult.ok) "successful" else "failed"}")
      }
    }
    ...
    mongoStore.findUsers(
      ...
      if (verbose)
        Logger.info(s"[User Deletion] ${users.length} registered users with old accounts found.\nSending eMails to users")

      val userIDs = users.map { user =>
        val mail = OldAccountEmail(user, registeredUserDeletionDateForEmail)
        mail.send
        if (verbose)
          Logger.info("[User Deletion] eMail sent to user: " + user.getUserData.nameLogin + " Last login: " + user.dateLastLogin.map(_.toString()).getOrElse("[no Date]"))
        user.userID
      }
