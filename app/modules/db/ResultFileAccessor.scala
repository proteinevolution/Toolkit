...
@Singleton
final class ResultFileAccessor @Inject()(constants : Constants,
    ...
    resultCache.get[JsValue](jobID).map {
      ...
      case None =>
        ...
        if ((constants.jobPath / jobID).exists && (constants.jobPath / jobID / "results").exists) {
          // Gather the files
          val files: List[File] = (constants.jobPath / jobID / "results").list.withFilter(_.extension.contains(".json")).toList
          Logger.info(s"[ResultFileAccessor] Loading files for $jobID: ${files.map(_.name).mkString(",")}")

          // Merge the results from the files
          val results: JsValue =
            Json.toJson(files.map { file =>
              file.nameWithoutExtension -> Json.parse(file.contentAsString)
            }.toMap[String, JsValue].updated("jobID", Json.toJson(jobID)))
