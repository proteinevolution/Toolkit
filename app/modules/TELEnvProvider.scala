...
class TELEnvProvider @Inject()(tv: TELEnv, configuration: Configuration)
  ...
  override def get(): TELEnv = {
    configuration
      ...
      .getOrElse {
        ...
        logger.warn(
          s"Key 'tel.env' was not found in configuration. Fall back to '$fallBackFile'"
        )
        fallBackFile
      }
      .toFile
      ...
      .foreach { file =>
        file.extension match {
          case Some(".prop") =>
            new PropFile(file.pathAsString, configuration).addObserver(tv)
