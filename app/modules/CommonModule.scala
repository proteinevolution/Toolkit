...
trait CommonModule extends ReactiveMongoComponents {
  ...
  /* Collections */
  protected lazy val hashCollection: Future[BSONCollection] =
  ...
  protected def writeArticleDatabase(featuredArticle: FeaturedArticle): Future[WriteResult] =
    articleCollection.flatMap(_.insert(featuredArticle))

  protected lazy val frontendJobCollection: Future[BSONCollection] =
    reactiveMongoApi.database.map(_.collection[BSONCollection]("frontendjobs"))
  protected lazy val jobAnnotationCollection: Future[BSONCollection] =
    reactiveMongoApi.database.map(_.collection[BSONCollection]("jobannotations"))
  // ResultfilesCollection
  protected lazy val resultCollection: Future[BSONCollection] =
  ...
  protected def findJobAnnotation(selector: BSONDocument): Future[Option[JobAnnotation]] =
  ...
  protected def eventLogCollection: Future[BSONCollection] =
    reactiveMongoApi.database.map(_.collection[BSONCollection]("jobevents"))

  protected def addJobLog(jobEventLog: JobEventLog): Future[WriteResult] =
  ...
  protected def statisticsCollection: Future[BSONCollection] =
  ...
  protected def addStatistic(toolStatistic: ToolStatistic): Future[WriteResult] =
