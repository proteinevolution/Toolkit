...
final class Backend @Inject()(webJarAssets: WebJarAssets,
  ...
  def index = Action { implicit ctx =>
    ...
    val session_id = Session.requestSessionID(ctx)
    val user_o : Option[User] = Session.getUser(session_id)
    if(ctx.headers.get("referer").getOrElse("").matches("http://" + ctx.host + "/@/backend.*") && authController.loggedOut.equals(false)) {

      NoCache(Ok(views.html.backend.backend(webJarAssets, views.html.backend.backend_maincontent(),"Backend", user_o)).withSession {
      Session.closeSessionRequest(ctx, session_id)
    })
    } else {
  ...
  def settings = Action { implicit ctx =>
    ...
    if(ctx.headers.get("referer").getOrElse("").matches("http://" + ctx.host + "/@/backend.*") && authController.loggedOut.equals(false)) {
      ...
      NoCache(Ok(views.html.backend.backend(webJarAssets, views.html.backend.settings(settingsController.clusterMode),"Backend", user_o)).withSession {
        Session.closeSessionRequest(ctx, session_id)
      })
